 ...
 * @version 1.0
 */
public class AjaxSignReportByAcceptanceAction extends BaseAction{
		...
		try{
			...
			for (String string : idArr) {
				...
				if(isNeedSign(report)){
					report.setAcceptanceUsers(getLoginUsers());
					report.setAcceptanceCheckTime(new Date(System.currentTimeMillis()));
					report.setRecordStatus(Constants.JT_REPORT_RECORD_STATUS_ACCEPTANCE_OR_TECH_CHECK);
				}
	...
	public boolean isNeedSign(JtReport report){
		...
		if(report.getTechnicianCheck() == Constants.CHECK_STATUS_NEED_CHECK && report.getTechnicianUsers() == null){
			return false;
		}
		if(report.getQcCheck() == Constants.CHECK_STATUS_NEED_CHECK && report.getQcUsers() == null){
			return false;
		}
		if(report.getAcceptanceCheck() == Constants.CHECK_STATUS_NEED_CHECK && report.getAcceptanceUsers() == null){
			return false;
		}
		return true;
	}
