 ...
 * @version 1.0
 */
public class AjaxSignReportByTechnicianAction extends BaseAction{
		...
		try{
			...
			for (String string : idArr) {
				...
				if(isNeedSign(report)){
					report.setTechnicianUsers(getLoginUsers());
					report.setTechnicianCheckTime(new Date(System.currentTimeMillis()));
	...
	public boolean isNeedSign(JtReport report){
		...
		if(report.getTechnicianCheck() == Constants.CHECK_STATUS_NOT_CHECK){
			return false;
		}
		return true;
	}
