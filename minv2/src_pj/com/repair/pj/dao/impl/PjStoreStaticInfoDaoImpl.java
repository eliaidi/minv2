...
* @Description: TODO(配件静态信息实现类)
* @author 唐鹏
* @version V1.0  
* @date 2015-8-31 下午05:57:24
 */
public class PjStoreStaticInfoDaoImpl extends DaoSupport<PjStoreStaticInfo> implements  PjStoreStaticInfoDao{
	...
	public SplitPageModel listPageByDto(PjStoreStaticInfoSearchDto dto){
		...
		if(StringUtil.isNotBlank(dto.getJcTypeValue())){
			hql.append(" and pssi.id in (select psjtpjfu.pjStoreStaticInfo.id from PjStaticJcTypePjFU psjtpjfu where " +
					" psjtpjfu.jcTypePjFirstUnit.id in (select jtpfu.id  from JcTypePjFirstUnit jtpfu where jtpfu.dictJcType.jcTypeValue = ?  ))");
			pars.add(dto.getJcTypeValue());
		}
