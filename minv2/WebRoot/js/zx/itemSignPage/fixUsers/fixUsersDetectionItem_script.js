$(function() {
	...
	$("#enable").click(function() {
		...
		var datePath = basePath + "zx/fixUsers/ajaxEnableZxFixItemRecord.action";
		$.ajax({
			"type": "post",
			"url": datePath,
			"data": data,
	...
	$("#enableAll").click(function() {
		top.Dialog.confirm("确定要一键启用项目？", function() {
			var itemType = $("#itemType").val();
			var planDateId = $("#planDateId").val();
			var data = {
				"planDateId": planDateId,
...
function cancelSign(recId) {
	top.Dialog.confirm("确认取消签认？", function() {
		var url = basePath + "zx/fixUsers/ajaxCancelSignZxFixItem.action";
		var parsData = {
			"id": recId
		};
		$.post(url, parsData, function(data) {
...
//填写检测值进行签认
function sign(id, fixCondition) {
	$("#table").each(function() {
		var fixCondition = $(this).find("#fixCondition").val();
		var url = basePath + "zx/fixUsers/ajaxSignZxFixItem.action";
		var itemType=$("#itemType").val();
		var parsData = {
			"ids": id,
