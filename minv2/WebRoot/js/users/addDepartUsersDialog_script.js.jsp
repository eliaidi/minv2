...
//	alert(parent.document.getElementById("frmright") );
	...
	<c:if test="${updateTitle == null  }">
		document.addUsersForm.action="addUsersByDepart!executeDepart.action";
	</c:if>
	
	<c:if test="${updateTitle != null  }">
		
		document.addUsersForm.action="updateUsersByIdByDepart!executeDepart.action";
	</c:if>
...
//打开添加页面选择角色子窗口
function checkRoleInUsersDepart(){
     var roleInUsersDeparttemp =  $("#roleId").val();
     top.Dialog.open({URL:"findRolePrivsInUsers!executeDepart.action?roleInUsersDepartList="+roleInUsersDeparttemp,Title:"请选择角色",ShowCloseButton:true});
