	...
	<div panelTitle="" showStatus="false" roller="true">
		<table class="tableStyle" transMode="true" style="font-size: 12px;">
			...
			<tr>
				<td>班组还是部门：</td>
				<td><c:choose>
						<c:when test="${usersPrivs.departProteamStatus=='1'}">部门</c:when>
						<c:when test="${usersPrivs.departProteamStatus=='2'}">班组</c:when>
						<c:when test="${usersPrivs.departProteamStatus=='3'}">部门和班组</c:when>
					</c:choose>
				</td>
				<td>是否启用：</td>
				<td><c:choose>
