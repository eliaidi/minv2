...
<html>
	...
	<head>
		...
		<!-- 引用以封装的复选框js								结束 -->
		<!-- 引用工人js											开始 -->
		<script type="text/javascript" src="${urlctx}js/report/page/qc/qcReportSign_script.js"></script>
		<!-- 引用工人js												结束 -->
		<!-- 引用工人table表格js											开始 -->
			...
			<c:forEach var="map" items="${jtReportMap}" varStatus="mStatus">
				...
				<c:forEach var="jtReport" items="${map.value }" varStatus="nStatus">
					<tr id="node-${mStatus.index}-${nStatus.index}" class="child-of-node-${mStatus.index }">
						<td>
							<c:if test="${jtReport.recordStatus <= qcCheckFlag && jtReport.qcCheck ==needCheck && jtReport.qcUsers == null }">
