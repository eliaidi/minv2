...
<html>
	...
	<body>
		<div class="box2" panelTitle="待签配件（工长）" roller="false">
			<table>
				<tr>
					<input type="hidden" value="${pjdStatus }" name="pjdStatus" id="pjdStatus" /> 配件名称：
					<font color="red">${pjName }<input type="hidden" value="${pjName}" name="pjName" id="pjName" /></font>
			...
			<table class="tableStyle" useMultColor="false" useCheckBox="true">
				...
				<c:forEach var="pjSSInfo" items="${spm.data}" varStatus="countPJ">
					<tr>
						...
						<td width="8%"><a target="_blank" href="${urlctx }pjFix/iframePage/checkFixPj/lead/leadSignManagerIframe.jsp?id=${pjSSInfo.id }"><span>进入签认</span></a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
	...
	<!-- 自定义js -->
	<script type="text/javascript" src="${ctx}/js/pjFix/checkFixPj/lead/leadFixingPjManagerIframe_script.js"></script>
