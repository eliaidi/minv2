...
<html>
			...
			<table class="tableStyle" useMultColor="false" useCheckBox="true">
				<c:forEach var="ssInfo" items="${spm.data}" varStatus="ssInfoIndex">
					<tr>
						...
						<td width="15%">
							<a href="queryPjSDInfoByFixUsersPjForPjFix.action?pjName=${ssInfo.pjName}&fixItemQueryFlag=<%=Constants.PJ_SDIS_DTO_FIX_ITEM_QUERY_FALG_UN_SIGNALL%>
						&roleType=<%=Constants.ROLE_TYPE_FIX_USERS%>" target="_blank">
						-<font color="red">${staySignPjMap[ssInfo.id]}</font>-个待签配件</a>
						</td>
						<td width="8%"><font color="orange">${fixingPjMap[ssInfo.id] }</font></td>
						<td width="8%"><font color="green">${standardPjMap[ssInfo.id] }</font></td>
					</tr>
				</c:forEach>
	...
	<!-- 引用检修员件按js文件				开始 -->
	<script type="text/javascript" src="${ctx}/js/pjFix/checkFixPj/fixUsers/dynamicPjListManagerIframe_script.js"></script>
	<!-- 引用检修员js文件				结束 -->
