...
<html>
	...
	<head>
		...
		<!--引入弹出窗组件end-->
		<script type="text/javascript" src="${urlctx }js/zx/itemSignPage/acceptance/acceptanceSignManagerIframe_script.js"></script>
	</head>

	<body>
		<input type="hidden" value="${urlctx}" id="basePath" />
		<input type="hidden" value="${planDate.id}" id="planDateId"/>
		<div class="box2" panelTitle="验收员签认" roller="false">
			<button id="signAll" ><span class="icon_ok">卡控全签</span></button>
		...
		<div id="scrollContent" class="border_gray">
			<table class="tableStyle" useMultColor="false" useCheckBox="true">
					<c:forEach items="${fixItemList }" var="zxFixItemRec">
						<tr>
							<th width="5%">
								<c:if test="${jcZxFixItemRec.acceptanceUsers != null}">
									<input type="checkbox" name="recId${jcZxFixItemRec.id}" value="${jcZxFixItemRec.id }" />
								</c:if>
							</th>
