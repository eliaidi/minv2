package com.repair.zx.action.jcZxFixItemRecord.technician;
...
public class AjaxSignZxFixItemRecordAction extends BaseAction{
	...
	public String execute() throws Exception {
		...
		try{
			...
			if(signAll){
				...
				list=jcZxFixItemRecordService.listByConditions(planDateId,null , null, Constants.ROLE_TYPE_TECHNICIAN);
			}else{
			...
			for (JcZxFixItemRecord record : list) {
				if(isNeedSign(record)){
					record.setTechnicianUsers(loginUsers);
					record.setTechnicianCheckTime(new Date());
					
					if(record.getQcCheck() == Constants.CHECK_STATUS_NEED_CHECK && record.getQcUsers() == null){
	...
	public boolean isNeedSign(JcZxFixItemRecord record){
		...
		if(Constants.CHECK_STATUS_NOT_CHECK.equals(record.getTechnicianCheck()) || record.getTechnicianUsers() != null){
			return false;
		}
		return true;
	}
