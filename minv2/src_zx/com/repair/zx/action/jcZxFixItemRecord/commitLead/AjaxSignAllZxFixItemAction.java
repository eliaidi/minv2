...
public class AjaxSignAllZxFixItemAction extends BaseAction{
	...
	public String execute() throws Exception {
		...
		try{
			List<JcZxFixItemRecord> list=jcZxFixItemRecordService.listByConditions(planDateId, null, null, Constants.ROLE_TYPE_COMMIT_LEAD);
		
			
			for (JcZxFixItemRecord record : list) {
				if(record.getCommitLeadUsers() != null){
					continue;
				}
				if(isNeedSign(record)){
					record.setCommitLeadUsers(loginUsers);
					record.setCommitLeadCheckTime(new Date());
	...
	public boolean isNeedSign(JcZxFixItemRecord record){
		...
		if(Constants.CHECK_STATUS_NOT_CHECK.equals(record.getCommitLeadCheck()) ){
			return false;
		}
		return true;
	}
